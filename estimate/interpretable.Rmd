...
p[["lasso_raw"]] <- lasso_result$plot +
  labs(
    title = "(a) Original Sparse LR",
    y = "Original Taxonomic Features"
  )
lasso_result
```
...
p[["stability_raw"]] <- compare_splits(xy, lasso_result$grid)$p +
  ...
  labs(title = "(e) Original Sparse LR Stability")
```
...
p[["lasso_features"]] <- lasso_result$p +
  labs(
    title = "(b) Featurized Sparse LR",
    y = "Derived Taxonomic Features"
    ) +
  scale_y_discrete(
...
p[["stability_features"]] <- comparison$plot +
  labs(title = "(f) Featurized Sparse LR Stability")
```
...
ggsave("~/Downloads/interpretable_figure.png", dpi = 400, width = 12, height = 6)
```
